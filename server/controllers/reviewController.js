...
async function recomputeLawyerRating(lawyerProfileId) {
  const [stats] = await Review.aggregate([
    { $match: { lawyerId: lawyerProfileId, isDeleted: false } },
    { $group: { _id: '$lawyerId', avgRating: { $avg: '$rating' }, count: { $sum: 1 } } },
  ]);

  await Lawyer.findByIdAndUpdate(lawyerProfileId, {
...
exports.createReview = asyncHandler(async (req, res) => {
  ...
  if (rating < 1 || rating > 5) {
    throw new AppError('Validation failed.', 400, [{ field: 'rating', message: 'Must be between 1 and 5.' }]);
  }
  ...
  // CHANGED: Review.lawyerId refs the Lawyer PROFILE (see models/review.js),
  // but booking.lawyerId is the lawyer's USER id — they are different ids.
  // Resolve the profile before writing the review, or every review ends
  // up pointing at a Lawyer document that doesn't exist.
  const lawyerProfile = await Lawyer.findOne({ userId: booking.lawyerId });
  if (!lawyerProfile) {
  ...
  await recomputeLawyerRating(lawyerProfile._id);

  return sendSuccess(res, 201, { review });
});

// GET /api/reviews/lawyer/:lawyerId — public. :lawyerId here is the Lawyer
// PROFILE id (matches Review.lawyerId and what publicLawyerController
// returns as `lawyer._id`).
